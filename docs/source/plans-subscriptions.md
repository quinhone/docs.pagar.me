...
charges | `null` | Número de vezes que o usuário deverá ser cobrado, **sem contar a cobrança inicial da assinatura no caso de pagamento com cartão de crédito** (caso devam existir 3 cobranças, o valor passado deve ser `2` no caso de cartão). `null` irá cobrar o usuário indefinidamente, ou até que o plano seja cancelado.
